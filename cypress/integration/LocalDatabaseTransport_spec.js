  ...
  describe('Check Internal Data API', async () => {
    ...
    it('Internal Data API - We must be able to edit the added user on local database', (done) => {
      ;(async function () {
        ...
        try {
          userDoc1.name = 'Orion'
          __id = await localDataTransport
            .table('User')
            .put({ __id: userAddedId1, ...userDoc1 })
        } catch (error) {
          // console.error(error)
          e = error
        }
    ...
    it('Internal Data API - We must be able to find the added user by it name', (done) => {
      ;(async function () {
        ...
        let e = null
        try {
          doc = await localDataTransport
            .table('User')
            .where({ name: userDoc1.name })
            .first()
          // console.error('>>>>>>>>>>doc:::::::::::', doc)
        } catch (error) {
          // console.error(error)
    ...
    it('Internal Data API - We must not be able to find the added user by it username because it is not indexed', (done) => {
      ;(async function () {
        ...
        try {
          doc = await localDataTransport
            .table('User')
            .where({ username: userDoc1.username })
            .first()
        } catch (error) {
          // console.error(error)
          e = error
        ...
        assert.equal(
          e.message,
          'KeyPath username on object store User is not indexed'
        )
        done()
      })()
    })
    ...
    it('Internal Data API - We must be able to bulk add users to local database', (done) => {
      ;(async function () {
        ...
        let e = null
        try {
          __ids = await localDataTransport
            .table('User')
            .bulkAdd([userDoc1, userDoc2], false, { allKeys: true })
    ...
    it('Internal Data API - We must be able to bulk edit users in local database', (done) => {
      ;(async function () {
        ...
        try {
          __ids = await localDataTransport
            .table('User')
            .bulkPut([userDoc1, userDoc2], false, { allKeys: true })
