...
Going against this implementation model, we intoduce a `proxy like` abstraction relying on IndexedDB as local (front end) database, which shipped on every modern browser. Aditionaly we abstract some kind of `data transporters` which is used `to send produced data to` and `to consume data from`. Every data change has an associated event object and action, which is not used only for `Event Sourcing`, but also to provide direct `bindings` for `decoupled View components`.
